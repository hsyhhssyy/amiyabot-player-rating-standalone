    ...
    <div v-for="card in cards" :key="card.id" class="card">
      ...
      <div class="card-text">
        <p class="line" style="font-size: 16px;">
          平均等级：&nbsp;精<span class="highlight">{{ card.avgEliteLevel }}&nbsp;-&nbsp;{{ card.avgLevel }}</span>级。平均 <span
            class="highlight"> {{ card.avgSkillLevel }}</span> 级技能
        </p>
        ...
        <p class="line" style="font-size: 16px;">
          持有率：<span class="highlight">{{ Math.round(card.ownRate*100) }}%</span>
        </p>
      </div>
    </div>
  </div>
...
import convertToDisplayFormat from '@/algorithms/date_process';
      ...
      try {
        ...
        const transformedData = data.map((item: any) => {
          return {
            ...
            ownRate: item.sampleCount / batchCount.value,
...
.card-image {
  height: 100%;
  /* Make the height match the card height */
  object-fit: cover;
